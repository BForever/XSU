...
u32 fs_open_dir(FS_FAT_DIR* dir, u8* filename)
{
    ...
    if (filename[0] != '/')
        goto fs_open_dir_error;

    dir->cur_sector = fs_data_cluster_to_sector(2);
    dir->loc = 0;
    dir->sec = 1;
    ...
    if (filename[1] != 0) {
        ...
        dir->cur_sector = fs_data_cluster_to_sector(get_start_cluster(&dir_find));
        ...
        for (sec = dir->sec; sec <= fat_info.BPB.attr.sectors_per_cluster; sec++) {
            ...
            if (sec < fat_info.BPB.attr.sectors_per_cluster) {
                ...
                if (next_clus <= fat_info.total_data_clusters + 1) {
                    dir->sec = 1;
                    dir->loc = 0;
                    dir->cur_sector = fs_data_cluster_to_sector(next_clus);
