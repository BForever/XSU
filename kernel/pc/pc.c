...
void pc_deletelist(task_struct *task)
{
    ...
    switch(task->state)
    {
        case PROC_STATE_RUNNING:break;
        case PROC_STATE_READY:list_del_init(&task->ready);break;
        case PROC_STATE_WAITING:list_del_init(&task->wait);break;
        case PROC_STATE_SLEEPING:list_del_init(&task->sleep);break;
        default:break;
    }
    enable_interrupts();
}
...
{
    unsigned int asid;
    asm volatile("mfc0 %0,$10":"=r"(asid));
    return asid&0xFF;
}
void setasid(unsigned int asid)
{
    unsigned int tmp;
    asm volatile("mfc0 %0,$10":"=r"(tmp));
    tmp &= ~0xFF;
    tmp |= (asid&0xFF);
...
task_struct* create_kthread(char* name,int level)
{
    ...
    asm volatile("la %0, _gp":"=r"(task->context.gp));//gp
...
void syscall_free(unsigned int status, unsigned int cause, context* pt_context)
{
    ...
    if(&vma->vma == current->vma.next)//code segment
    {
        ...
        while(1);//try to free code space
    }
    if(vma)
    {
        kfree((void*)vma->pa);
